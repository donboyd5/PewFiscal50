...
rle <- usmdb %>%
  ...
  mutate(rle_cut=rle_cut,
         rle_age=age_1 + (ex_1 - rle_age) / (ex_1 - ex_2))
rle

rle %>% filter(stabbr=="NY")

rle %>% 
  filter(stabbr %in% c("AL", "NY")) %>%
  filter(year >= 2000) %>%
...
pop_rle <- pop %>%
  ...
  mutate(share_gtrle=pmax((age + 1) - rle_age, 0) %>% pmin(., 1),
...
pop_rle2 <- pop_rle %>%
  ...
  summarise(pop_tot=sum(pop),
            ...
            work_pop=sum((age>=21) * pop_ltrle),
            rle_pop=sum(pop_gtrle)) %>%
...
```{r meas_xwalk}
xwalk <- tribble(
  ~name, ~desc, ~num, ~den, ~note,
  "oadr", "Old age dependency ratio", "Older population (65+)", "Working age population (21-64)", NA,
  "tdr", "Total dependency ratio", "Older plus younger population (65+ plus age < 21)", "Working age population (21-64)", NA,
  "le65", "Life expectancy at age 65", NA, NA, NA,
  "le85", "Life expectancy at age 85", NA, NA, NA,
  "rle15", "Age at which remaining life expectancy is 15 years", NA, NA, NA,
  "oalfdr", "Old age labor force dependency ratio", "Older population (65+) NOT in labor force", "Working-age labor force (20-64)", "aka ADR",
  "rlepct_pop", "Population RLE < 15 as % of total population", NA, NA, NA,
  "rlepct_work", "Population RLE < 15 as % of working-age population (21-RLE)", NA, NA, NA,
  "disb_rate", "Disabled population as % of total population", NA, NA, NA,
  "old_rate", "Old (65+) population as % of total population", NA, NA, NA,
  "old_disb_rate", "Old (65+) plus disabled population as % of total population", NA, NA, NA,
  "old_rate_wp", "Old (65+) population as % of working age (21-64) population", NA, NA, NA,
  "old_disb_rate_wp", "Old (65+) plus disabled population as % of working age (21-64) population", NA, NA, NA
  )
xwalk
...
p <- agedr %>%
  ...
  scale_y_continuous("Percent", breaks=seq(0, 50, 2)) +
  scale_x_continuous(name=NULL, breaks=seq(2000, 2020, 2)) +
  ggtitle("Old-age dependency ratio, selected states",
          subtitle="Population 65+ as % of working-age population aged 21-64") +
  theme_bw() +
  theme(legend.title = element_blank())
p
ggsave(here::here(resdir, "oadr.png"), plot=p, width=10, height=8, scale=1)  
